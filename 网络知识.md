...
- [Web基础](#web基础)
    ...
    - [37. 幂等](#37-幂等)
    - [38. 为什么要有TIME_WAIT这个状态](#38-为什么要有time_wait这个状态)
- [Django](#django)
    - [1. Django](#1-django)
...
> **长连接**：HTTP1.1支持长连接（Persistent Connection）和请求的流水线处理（Pipelining），在一个TCP连接上可以传送多个HTTP请求和响应，减少了建立和关闭连接的消耗和延迟，在HTTP1.1中默认开启`Connection: keep-alive`，一定程度上弥补了HTTP1.0每次请求都要创建连接的缺点。
