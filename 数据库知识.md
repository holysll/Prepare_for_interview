...
- [数据库常识](#数据库常识)
    - [1. 什么是索引](#1-什么是索引)
    - [2. 索引的数据结构](#2-索引的数据结构)
    - [3. 聚簇索引与非聚簇索引](#3-聚簇索引与非聚簇索引)
    - [4. 什么是索引覆盖](#4-什么是索引覆盖)
    - [5. 什么是回表查询](#5-什么是回表查询)
    - [6. 建立索引需要考虑的因素](#6-建立索引需要考虑的因素)
    - [7. 联合索引](#7-联合索引)
    - [8. 查看索引是否被使用到](#8-查看索引是否被使用到)
    - [9. 为什么要尽量设定一个主键](#9-为什么要尽量设定一个主键)
    - [10. 主键使用自增ID还是UUID](#10-主键使用自增id还是uuid)
    - [11. 字段为什么要求定义为NOT NULL](#11-字段为什么要求定义为not-null)
    - [12. 如果要存储用户的密码散列，应该使用什么字段进行存储](#12-如果要存储用户的密码散列应该使用什么字段进行存储)
    - [13. 什么是事务](#13-什么是事务)
    - [14. 事务的特性](#14-事务的特性)
    - [15. MySql的事务隔离级别](#15-mysql的事务隔离级别)
    - [16. 同时多个事务并发执行可能造成的问题](#16-同时多个事务并发执行可能造成的问题)
    - [17. 不可重复读和幻读的区别](#17-不可重复读和幻读的区别)
    - [18. 共享锁与排他锁](#18-共享锁与排他锁)
    - [19. 表级锁、行级锁、页级锁](#19-表级锁行级锁页级锁)
    - [20. 死锁](#20-死锁)
    - [21. 什么是悲观锁](#21-什么是悲观锁)
    - [22. 什么是乐观锁](#22-什么是乐观锁)
    - [23. MySQL支持哪些存储引擎](#23-mysql支持哪些存储引擎)
    ...
    - [24. MyISAM和InnoDB的区别](#24-myisam和innodb的区别)
    - [25. 存储过程](#25-存储过程)
    - [26. 什么是视图](#26-什么是视图)
    - [27. 什么是游标](#27-什么是游标)
    - [28. 触发器](#28-触发器)
    - [29. drop、truncate、delete区别](#29-droptruncatedelete区别)
    - [30. 临时表](#30-临时表)
    - [31. 数据库范式](#31-数据库范式)
    - [32. 关系型数据库与非关系型数据库优势和区别](#32-关系型数据库与非关系型数据库优势和区别)
    - [33. SQL语言分类](#33-sql语言分类)
    - [34. count(*)、count(1)和count(column)的区别](#34-countcount1和countcolumn的区别)
    - [35. like、百分号和下划线的区别](#35-like百分号和下划线的区别)
    - [36. 最左前缀原则](#36-最左前缀原则)
    - [37. 超大分页处理](#37-超大分页处理)
    - [38. 慢查询优化](#38-慢查询优化)
    - [39. 横向分表和纵向分表](#39-横向分表和纵向分表)
    - [40. B+树和B树的区别](#40-b树和b树的区别)
    - [41. 查询语句中用到的关键词](#41-查询语句中用到的关键词)
    - [42. 使用explain优化SQL和索引](#42-使用explain优化sql和索引)
    - [43. 数据库组主从复制的方式](#43-数据库组主从复制的方式)
    - [44. 数据库崩溃时事务的恢复机制（REDO日志和UNDO日志）](#44-数据库崩溃时事务的恢复机制redo日志和undo日志)
- [Mysql数据库（端口：3306）](#mysql数据库端口3306)
    - [1. 创建数据库](#1-创建数据库)
    - [2. 用户管理](#2-用户管理)
    - [3. 授权管理](#3-授权管理)
    - [4. 修改表的列](#4-修改表的列)
    ...
    - [11. 关联查询](#11-关联查询)
    - [12. 组合](#12-组合)
    - [13. LIMIT的用法](#13-limit的用法)
    - [14. case when](#14-case-when)
    - [15. mysql 常用数据类型](#15-mysql-常用数据类型)
    - [16. MySQL读取数据的基本单位](#16-mysql读取数据的基本单位)
    - [17. 为什么mysql事务回滚后，自增ID依然自增](#17-为什么mysql事务回滚后自增id依然自增)
    - [18. mysql的主从复制的实现过程](#18-mysql的主从复制的实现过程)
    - [19. mysql高并发环境解决方案](#19-mysql高并发环境解决方案)
- [Redis数据库（端口：6379）](#redis数据库端口6379)
    - [1. 什么是Redis](#1-什么是redis)
    - [2. redis的数据类型](#2-redis的数据类型)
    - [3. 两种持久化方式](#3-两种持久化方式)
    - [4. RDB与AOF比较](#4-rdb与aof比较)
    - [5. 如果AOF文件出现被写坏的情况，该怎么处理](#5-如果aof文件出现被写坏的情况该怎么处理)
    - [6. redis 和 memcached 的主要区别](#6-redis-和-memcached-的主要区别)
    - [7. 事务处理](#7-事务处理)
    - [8. Redis常见的架构模式](#8-redis常见的架构模式)
        - [(1) 单机模式](#1-单机模式)
        - [(2) 主从复制模式](#2-主从复制模式)
        - [(3) 哨兵模式](#3-哨兵模式)
        - [(4) 集群模式](#4-集群模式)
    - [9. Redis如何实现分布式锁](#9-redis如何实现分布式锁)
    - [10. 缓存使用场景](#10-缓存使用场景)
    - [11. 常用的缓存使用模式](#11-常用的缓存使用模式)
    - [12. 什么是缓存穿透](#12-什么是缓存穿透)
    - [13. 什么是缓存击穿](#13-什么是缓存击穿)
    - [14. 什么是缓存雪崩](#14-什么是缓存雪崩)
    - [15. 连接Redis](#15-连接redis)
    - [16. Redis异步队列](#16-redis异步队列)
    - [17. 生产一次消费多次](#17-生产一次消费多次)
    - [18. Redis分区](#18-redis分区)
    - [18. mySQL里有2000w数据，redis中只存20w的数据，如何保证redis中的数据都是热点数据](#18-mysql里有2000w数据redis中只存20w的数据如何保证redis中的数据都是热点数据)
- [MongoDB数据库（端口：27017）](#mongodb数据库端口27017)
    - [1. MongoDB常用的操作](#1-mongodb常用的操作)
...
> 推荐使用自增ID，不要使用UUID（通用唯一识别码，Universally Unique Identifier）。因为在`InnoDB`存储引擎中，主键索引是作为聚簇索引存在的，也就是说主键索引的B+树叶子节点上存储了主键索引以及全部的数据，如果主键索引是自增ID,那么只需要不断向后排序即可；如果用UUID，由于到来的ID与原来的大小不确定，会造成非常多的数据插入、数据移动，然后导致产生很多的内存碎片，进而造成插入性能的下降。
...
> 在MySQL中只有使用了`Innodb`数据库引擎的数据库或表才支持事务；事务是数据库并发控制的基本单位；事务可以用来管理`INSERT/UPDATE/DELETE`操作语句；事务处理可以用来维护数据库的完整性，保证成批的SQL语句要么全部执行，要么全部不执行（回滚）。
...
- `count(*)`：对行的数目进行计算，包含NULL，跟`count(非主键)`一样，扫描整个表；
- `count(1)`：与`count(主键)`是一样的，只扫描主键；
...
**[以上来源于《菜鸟教程--Redis简介》](https://www.runoob.com/redis/redis-intro.html)**
...
> Redis 列表是简单的字符串列表，按照插入顺序排序，底层实现是链表，对于一个具有几百上千万个元素的list来说，在头部和尾部插入一个新元素，其时间复杂度是常数级别的，插入速度也是不变的。弊端就是链表型的list比数组型的list定位更慢。list主要应用在消息队列上，可以确保先后顺序，还可以利用`lrange`实现分页功能，博客系统中评论也可以存入一个单独的list中。
...
> Append Only File意思是只允许最佳不允许改写的文件，AOF的方式是将执行过程的所有写指令记录下来，在数据回复时按照冲钱到后顺序在将指令执行一遍。通过配置redis.conf中的`appendonly yes`就可以打开AOF功能，每当服务器执行定时任务或者函数时，都会调用`flushAppendOnlyFile()`函数，根据条件将aof_buf中的缓存写到AOF文件中，而后调用`fsync()`或 `fdatasync()`函数将AOF保存到磁盘中。其中AOF默认持久化的策略是每秒钟fsync一次，所以对数据安全更加有保障。
...
- redis内部使用的是文件事件处理器`file event handler`，是单线程模型，处理效率高。文件事件处理器的结构包含多个socket、IO多路复用程序（非阻塞）、文件事件分派器、事件处理器四个部分。
